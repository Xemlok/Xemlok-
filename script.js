...
document.addEventListener('DOMContentLoaded', () => {
    // Analyze Token button in hero – скроллим к #analytics
    ...
    // Learn More button – скроллим к #features
    ...
    // Navigate to Chrome Web Store from Analyze button + Enter в инпуте
    const goAnalyseBtn = document.querySelector('.btn-go-analyse');
    const searchInput = document.querySelector('.token-search');
    const storeUrl = 'https://chromewebstore.google.com/detail/xemlok/pfdbbahlidmeoblljennfdbngcmbaepc';
    ...
    if (goAnalyseBtn && searchInput) {
        ...
        goAnalyseBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            openStore();
        });

        // Allow Enter key to open store
...
document.addEventListener('DOMContentLoaded', () => {
    const animatedElements = document.querySelectorAll(
        '.feature-column, .stat-card, .stat-item, .about, .features, .analytics-demo, .about-content h2, .about-content p, .search-container, .stats-grid'
    );
